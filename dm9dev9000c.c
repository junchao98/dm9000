...
struct net_device * __init dmfe_probe(void)
{
	...
	if(!dev)
		return ERR_PTR(-ENOMEM);
     	...
     	//SET_MODULE_OWNER(dev);	//lornyin@126.com
	err = dmfe_probe1(dev);
	if (err)
		goto out;
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,5,0)
...
static const struct net_device_ops dm9000_netdev_ops = {          //lornyin@126.com
	.ndo_open		= &dmfe_open,
	.ndo_stop		=  &dmfe_stop,
...
int __init dmfe_probe1(struct net_device *dev)
{
	...
	do {
		...
		if (id_val == DM9KS_ID || id_val == DM9010_ID) {
			...
			//dev->open 		= &dmfe_open;         //lornyin@126.com
			...
			dev->netdev_ops	= &dm9000_netdev_ops;       //lornyin@126.com
...
{
	board_info_t *db =  netdev_priv(dev);
	//struct dev_mc_list *mcptr = dev->mc_list;   //lornyin
	...
	/* the multicast address in Hash Table : 64 bits */    //lornyin@126.com
	/*
	for (i = 0; i < mc_cnt; i++, mcptr = mcptr->next) {
		...
		netdev_for_each_mc_addr(ha, dev) {                    //lornyin@126.com
...
//	.get_rx_csum		= dmfe_get_rx_csum,        //lornyin@126.com
